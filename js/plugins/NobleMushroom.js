...
// ----------------------------------------------------------------------------
// Version
// 1.7.3 2018/12/14 ロード、クイックロード时に决定ボタンを押し続けているとロード処理が缲り返されてしまう问题を修正
//                  アイコン画像が自动改行时に考虑されない问题を修正
// 1.7.2 2018/09/25 MessageSkip.jsと组み合わせたときにオート待机フレームの际に取得するテキスト文字数が正しく取得できていなかった竞合を解消
// 1.7.1 2017/12/30 相対フォントサイズにマイナス値を设定しても反映されなかった问题を修正
// 1.7.0 2017/12/17 ノベルメッセージの縦书きの対応
// 1.6.1 2017/10/29 场所移动のフェードアウト中にポーズメニューを押下すると、移动后にイベントが进まなくなる不具合を修正
// 1.6.0 2017/10/07 自动改行が有効な场合、自动改行位置と入力した改行が重なったときに2回改行されてしまう问题を修正
//                  パラメータの型指定机能に対応
// 1.5.2 2017/05/25 1.5.1で オートセーブ无効时にイベントメニューからセーブすると空でセーブされてしまう问题を修正
// 1.5.1 2017/04/03 オートセーブ无効时でも一部の条件でオートセーブされていた问题を修正
// 1.5.0 2016/12/05 高速でメッセージを送った场合に顔グラフィックを表示しようとするとエラーになる场合がある不具合を修正
//                  ノベル表示中に选択肢ウィンドウと数値入力ウィンドウの表示位置を调整できる机能を追加
// 1.4.0 2016/11/11 オートセーブ有効时、一定の手顺を踏むとセーブデータをロードできなくなる不具合を修正
//                  选択肢表示后、直后にメッセージ表示がない场合でもウィンドウを闭じないよう修正
// 1.3.0 2016/09/28 ウィンドウ枠を自由に调整できる机能を追加
//                  ウィンドウの枠外をクリックしたときに文章を进めない设定を追加
//                  フォント名を指定できる机能を追加（デバイスにインストールされているフォントを使用します）
//                  明朝体、ゴシック体をどちらも使わない场合にフォントが小さく表示されてしまう问题を修正
//                  Edgeでフォントが小さく表示されてしまう问题を修正
// 1.2.0 2016/09/22 セーブファイルをひとつしか作成できない制约を解消
//                  オートセーブを単独で动作するよう修正
//                  イベント中にセーブやロードができるポーズメニューを追加
//                  クイックセーブ＆ロード机能を追加
//                  セーブファイルに表示できるチャプター表示机能を追加
// 1.1.0 2016/09/20 画面サイズ変更およびモバイル用の画面サイズ设定を追加
// 1.0.0 2016/08/16 初版
// ----------------------------------------------------------------------------
...
/*:
 * @plugindesc 视觉小说集合插件
 * @author トリアコンタン ( 翻译 : ReIris )
 *
 * @param InitialViewType
 * @text 显示类型
 * @desc 讯息显示的类型预设值。(0:正常 1:小说)
 * @default 1
 * @type select
 * @option Normal
 * @value 0
 ...
 * @param WaitByCommand
 * @text 显示停顿图示
 * @desc 事件命令中「显示文字」等待每个讯息后面的停顿图示。(ON/OFF)
 * @default true
 * @type boolean
 *
 * @param VariableSpeed
 * @text 显示速度变数
 * @desc 储存讯息显示数度的变数ID。变数内的值为显示 1 文字的待机时间。
 * @default 1
 * @type variable
 *
 * @param InitialSpeed
 ...
 * @text 点击瞬间显示
 * @desc 在讯息中按下确定键或左键点击瞬间显示全部讯息。(ON/OFF)
 * @default true
 * @type boolean
 *
 * @param ClickInFrame
 * @text 是否限制点击框内
 * @desc 当滑鼠点击操作在讯息窗口的范围内时才有效。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @param AutoWordWrap
 * @text 自动换行
 * @desc 当讯息超过窗口时自动换行。(ON/OFF)
 * @default true
 * @type boolean
 *
 * @param RelativeFontSize
 * @text 小说窗口字体大小
 * @desc 小说窗口的字体大小。它是默认字体大小的相对值。
 * @default 6
 * @type number
 * @min -32
 * @max 32
 ...
 * @param ViewMincho
 * @text 是否优先使用明朝字体
 * @desc 如果设备上安装了明朝类型字体，则会优先使用它。(ON/OFF)
 * @default true
 * @type boolean
 *
 * @param ViewGothic
 * @text 是否优先使用哥特字体
 * @desc 如果设备上安装了哥特式字体，则会优先使用。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @param ViewCustomFont
 * @text 是否优先使用指定字体
 * @desc 如果设备上安装了指定的字体，则会优先使用该字体。(複数指定使用逗号[,]区分)
 * @default
 *
 * @param SelectionPrefix
 * @text 选项顺序前缀
 * @desc 选择选项的顺序前缀。(0:不使用 1:字母 2:数字)
 * @default 0
 * @type select
 * @option No Use
 * @value 0
 ...
 * @param ScreenWidth
 * @text 画面宽度
 * @desc 水平方向的画面尺寸。指定为 0 则不变更。
 * @default 0
 * @type number
 *
 * @param ScreenHeight
 * @text 画面高度
 * @desc 垂直方向的画面尺寸。指定为 0 则不变更。
 * @default 0
 * @type number
 *
 * @param ScreenWidthMobile
 * @text 手机画面宽度
 * @desc 使用手机等时，水平方向的画面尺寸。指定为 0 则不变更。
 * @default 0
 * @type number
 *
 * @param ScreenHeightMobile
 * @text 手机画面高度
 * @desc 使用手机等时，垂直方向的画面尺寸。指定为 0 则不变更。
 * @default 0
 * @type number
 *
 * @param MobileMode
 * @text 手机模式
 * @desc 可以在PC上以手机模式运行。它是一个主要用于测试的选项，但存在声音不响的限制。
 * @default false
 * @type boolean
 *
 * @param AutoSave
 * @text 自动存档
 * @desc 游戏进度状态将自动保存。当使用声音小说作为迷你游戏时，建议为OFF。
 * @default true
 * @type boolean
 *
 * @param CanPause
 * @text 呼叫暂停选单
 * @desc 显示类型为小说模式时，事件执行中使用取消按键将呼出暂停选单，可以进行存档或读取。
 * @default true
 * @type boolean
 *
 * @param NameAutoSave
 ...
 * @text 快速存档文本
 * @desc 选单中的「快速存档」的命令名称。
 * @default 快速存档
 *
 * @param CommandQuickLoad
 * @text 快速读取文本
 * @desc 选单中的「快速读取」的命令名称。
 * @default 快速读取
 *
 * @param VerticalWriting
 * @text 直行显示
 * @desc 小说显示文字为直行显示。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @help 它是一个基础插件，使用 RPGMakerMV 轻鬆创建视觉小说。
 * 使用时，讯息窗口的显示变为整个画面，
 * 并且显示的消息将被留存在画面上而不被删除。
 *
 * 在显示小说模式时按下取消按钮或右键单击（多点触控）时，
 * 事件进度停止，并显示暂停选单。
 * 暂停选单可以执行以下操作
 *
 * Save
 ...
 *
 * 控制字元
 * \UL    # 等待显示文字立即取消等待并将事件命令前进到下一个。
 * \WC    # 关闭窗口，它将在此执行自动存档。
 * \MS[n] # 暂时将文字显示速度更改为「n」帧。
 *
 * 插件命令
 *  从事件命令中「插件命令」执行。
 *  （参数指定使用半形空格区分）
 *
 * NM_CHANGE_TYPE 1    # 更改文字的显示类型。
 *                       移动地点后将反应设置。
 * NM_RE_WAIT          # 启用控制字元[\UL]已解除的显示文字等待。
 * NM_CLOSE            # 明确关闭窗口。
 * NM_SETTING_FIXED    # 将窗口的显示设置固定为当前设置。
 *                       在固定状态下，忽略后续文字的显示设置。
 * NM_SETTING_RELEASE  # 恢复固定窗口的显示设置。
 * NM_INPUT_NAME 1     # 显示角色ID[1]的名称输入窗口(※)
 * ※这个命令在RPGアツマール无法使用。
 *
 * NM_SET_CHAPTER A    # 设置要输出到存档的章节标题。
 * NM_AUTO_SAVE        # 执行自动存档。
 *
 * ・通过指定 X 座标， Y 座标，宽度和高度来调整小说窗口的显示位置。
 * 　如果未指定参数，则显示位置将返回到默认值。
 * NM_SET_RECT_NOVEL_WINDOW 0 0 600 300    # 设置窗口矩形。
 *
 * ・通过指定 X 座标和 Y 座标来调整小说窗口选项的中心坐标。
 * 　如果未指定参数，则显示位置将返回到默认值。
 * NM_SET_RECT_NOVEL_COMMAND 0 0 # 设置小说选项的中心坐标。
 *
 * ・NM_CHANGE_TYPE
 * 变更讯息显示的类型。类型如以下。
 *
 * 0 : 通常讯息显示。
 * 1 : 显示为小说模式。讯息将显示在全画面上，并且留存于画面。
 *     自动调整显示方式，包括选择项和数字输入。
 *
 * 请注意，在移动到该地点之前，不会反应设置更改。
 *
 * ・NM_SETTING_FIXED
 * 将窗口的设置固定为当前设置。设置目标如下。
 *
 * 1. 脸图设定
 * 2. 类型(窗口、暗化、透明)
 * 3. 位置(上　中　下)
 *
 * 除非固定，否则文本显示中的设置将被忽略。
 * 如果重新启用，请执行 NM_SETTING_RELEASE 取消固定设置。
 *
 * ・NM_INPUT_NAME
 * 不使用专用画面的简易版输入名称的处理。
 * 如果为第二个参数设置了文字，则会在对话框中显示说明。
 * 如果输入值保留为空或取消，则不会更改名称。
 * (例)
 * NM_INPUT_NAME 1 名前を入力してください。
 *
 * ・NM_SET_RECT_NOVEL_WINDOW
 * 指定整个画面上显示的小说窗口的显示位置（X，Y，宽度，高度）。
 * 如果未指定参数，则返回默认大小。
 *
 * 利用规约：
 *  作者に无断で改変、再配布が可能で、利用形态（商用、18禁利用等）
 *  についても制限はありません。
 *  このプラグインはもうあなたのものです。
 */
...
(function() {
    ...
    var paramRapidShowClick     = getParamBoolean(['RapidShowClick', 'クリック瞬间表示']);
    var paramClickInFrame       = getParamBoolean(['ClickInFrame', 'クリック范囲限定']);
    ...
    var paramAutoWordWrap       = getParamBoolean(['AutoWordWrap', '自动改行']);
    ...
    var paramSelectionPrefix    = getParamNumber(['SelectionPrefix', '选択肢接头辞'], 0, 2);
    ...
    var paramVerticalWriting    = getParamBoolean(['VerticalWriting', '縦书き']);
    ...
    //=============================================================================
    // Scene_Load
    //  オートセーブ有効时にオートセーブ用のインデックスを有効にします。
    //=============================================================================
